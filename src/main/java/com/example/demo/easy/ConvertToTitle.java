...
public class ConvertToTitle {
   ...
   * <p>
   * 对于一般性的进制转换题目，只需要不断地对 columnNumber 进行 % 运算取得最后一位， 然后对 columnNumber 进行 / 运算，将已经取得的位数去掉，直到
   * columnNumber 为 0 即可。
   * <p>
   ...
   * @return int
   * @Author YJY
   * @Description 进制转换  和原题是反转题目 也许你没专门做过「进制转换」类的题目，但相信你确实在 1010 进制中使用过如下的「进制转换」手段。
   * <p>
   * 如果题目是 10 进制转换，那么你会很容易想到如下转换过程： 从高位向低位处理，起始让 ans为 0，每次使用当前位数值更新 ans，更新规则为 ans = ans * 10 +
   * val_i
   * <p>
   * 举个🌰，假设存在某个十进制数字，编码为 ABCD（字母与数字的映射关系与本题相同），转换过程如下：
   * <p>
   * ans = 0 ans = ans * 10 + 1 => A ans = ans * 10 + 2 => B ans = ans * 10 + 3 => C ans = ans * 10
   * + 4=> D
   * <p>
